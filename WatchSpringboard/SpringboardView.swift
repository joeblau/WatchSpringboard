...
class SpringboardView: UIScrollView, UIScrollViewDelegate {
  ...
  func showAllContent(animated: Bool) {
    var contentRectInContentSpace = fullContentRectInContentSpace()
  ...
  func didZoomGesture(sender: UITapGestureRecognizer) {
    ...
    if zoomScale >= minimumZoomLevelInteraction && zoomScale != minimumZoomScale {
      ...
      UIView.animateWithDuration(0.5, animations: { () -> Void in
        self.centerOn(targetIndex, zoomScale: maxZoom, animated: false)
        self.layoutIfNeeded()
        }, completion: nil)
    }
  }
  ...
  func closestIndexToPointInContent(pointInContent: CGPoint) -> UInt {
    ...
    for view in itemViews {
      var center = view.center
      let potentialDistance = PointDistance(center.x, center.y, pointInContent.x, pointInContent.y)
      
      if (potentialDistance < distance || hasItem == false) {
        hasItem = true
  ...
  override func layoutSubviews() {
    ...
    if isMinZoomLevelDirty {
      ...
      var newZoom: CGFloat = max(zoomScale, newMinimumZoomScale)
